...
dispatch do L3 ACDC e L5 HRR cleanup ficaram **inacessíveis** em
qualquer clone novo do fork.

## Solução

Esta pasta contém os dois patches de dispatch exportados via
`git format-patch` a partir dos commits órfãos. O script
`scripts/apply-dispatch-patches.sh` os aplica em ordem (L3 primeiro,
L5 depois — L5 depende do guard `#if` que L3 adiciona) após o
`git submodule update --init`.

## Patches

| Arquivo | Commit original | Linhas | O que faz |
|---------|-----------------|--------|-----------|
| `01-L3-ACDC-FFN-dispatch.patch` | `707f3162` (L3 ACDC dispatch) | 162 | Adiciona `llm_build_ffn_acdc_bitnet` e o branch `BITNET_ACDC_FFN=1` no call site FFN BitNet-específico; estende o guard `#if` para incluir `BITNET_L3_ACDC` |
| `02-L5-HRR-cleanup-dispatch.patch` | `3dfc2dfa` (L5 HRR cleanup dispatch) | 16 | Adiciona branch `BITNET_HRR_ATTN_CLEANUP=N` no call site KQV BitNet-específico; estende o guard `#if` para incluir `BITNET_L5_HRR` |
...
- Os patches são **acumulativos**: L5 assume que L3 já foi aplicado.
  O script aplica nessa ordem automaticamente.
