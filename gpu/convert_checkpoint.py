...
def convert_ts_checkpoint(
	*,
	input_path: str = "",
	config: Optional[model.ModelArgs] = None,
) -> None:
	...
	def quant_weight_int8(weight):
		# Convert to float for mean calculation to avoid "could not infer output dtype" for Byte/Int tensors
		w_float = weight.float()
		s = 1.0 / w_float.abs().mean().clamp_(min=1e-5)
		new_weight = (w_float * s).round().clamp(-1, 1).to(torch.int8)
	...
	def quant_weight_fp16(weight):
		w_float = weight.float()
		s = 1.0 / w_float.abs().mean().clamp_(min=1e-5)
		new_weight = (w_float * s).round().clamp(-1, 1) / s
...
if __name__ == "__main__":
	...
	parser.add_argument("--ffn_dim", type=int, default=6912)

	args = parser.parse_args()
	config = model.ModelArgs(dim=args.dim, n_layers=args.n_layers, n_heads=args.n_heads, n_kv_heads=args.n_kv_heads, ffn_dim=args.ffn_dim)
