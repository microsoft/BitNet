...
transformer_configs = {
	...
	"Llama3-8B": dict(n_layer=32, n_head=32, dim=4096, vocab_size=128256, n_local_heads=8, intermediate_size=14336),
}
...
def convert_back(
	...
	for layer in range(cfg.n_layer):
		...
		if f"{base}self_attn.attn_sub_norm.weight" in st_dict:
			recovered[f"layers.{layer}.attention.attn_sub_norm.weight"] = st_dict[f"{base}self_attn.attn_sub_norm.weight"]
		if f"{base}mlp.ffn_sub_norm.weight" in st_dict:
			recovered[f"layers.{layer}.feed_forward.ffn_sub_norm.weight"] = st_dict[f"{base}mlp.ffn_sub_norm.weight"]

		# Handle weight scales for Llama3 (they are per-layer/per-tensor in some BitNet implementations)
		# In this specific repo's model.py, they are expected as weight_scale in BitLinearKernel
		# We'll map them if they exist
		for proj in ["self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj", "self_attn.o_proj", "mlp.gate_proj", "mlp.up_proj", "mlp.down_proj"]:
			scale_key = f"{base}{proj}.weight_scale"
			if scale_key in st_dict:
				# model.py expects a unified weight_scale per linear.
				# For WQKV we might need logic, but for now let's just save them so convert_checkpoint can see them
				recovered[f"layers.{layer}.{proj}.weight_scale"] = st_dict[scale_key]
...
if __name__ == "__main__":
	...
	parser.add_argument("--safetensors_file", type=str, required=True, help="Path to input .safetensors file")
	parser.add_argument("--output", type=str, default="./checkpoints/model_state.pt", help="Path to output .pt file")
	parser.add_argument("--model_name", type=str, default="2B", help="Model configuration name to use (e.g. 2B)")
