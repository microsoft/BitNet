...
class PerplexityTester:
    ...
    def quantize_embedding(self, embedding_type, output_suffix):
        ...
        try:
            ...
            print("❌ Quantization timeout (exceeded 10 minutes)")
            return None
        except Exception as e:
    ...
    def run_all_tests(self, threads=16, ctx_size=512):
        ...
        if not datasets:
            ...
            print("   Make sure each dataset directory has a test.txt file")
            return
        ...
        if self.quick_mode:
            ...
            print("   - Testing all datasets with first 4096 characters only")
            print(f"   - Using reduced context size: {ctx_size}")

        # Determine models to test
        if self.test_embeddings:
            print(f"\n{'='*80}")
            print("🧪 EMBEDDING QUANTIZATION TEST MODE")
            print(f"{'='*80}")
    ...
    def save_results(self):
        ...
        with open(log_file, 'w') as f:
            f.write("Perplexity Test Results\n")
            f.write(f"{'='*80}\n")
