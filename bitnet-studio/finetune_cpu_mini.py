"""Fine-tune CPU piloto mínimo — 5 exemplos, 5 steps, seq 64."""
import json, torch
...
OUTPUT = "adapters/f3b-ptbr-tools-cpu-mini"
...
# Apenas 5 primeiros exemplos
rows = []
with open("data/ptbr_tools_train.jsonl", encoding="utf-8") as f:
...
print(f"Dataset mini: {len(texts)} exemplos")

ds = Dataset.from_dict({"text": texts}).map(
    lambda b: tok(b["text"], truncation=True, max_length=64, padding=False),
